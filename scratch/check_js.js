...
async function deleteSparepart(id){
  ...
  showConfirmPopup('ðŸ—‘ï¸ Hapus Item', 
    `Hapus master data item: <b>${sp.nama}</b> (Spek: ${sp.spek})?<br><br><span style="color:var(--danger);font-size:11px;">âš ï¸ Tindakan ini akan gagal jika item masih memiliki riwayat pemakaian.</span>`,
    async () => {
      // DELETE via backend
      const result = await apiFetch(`/api/inventory/${id}`, { method: 'DELETE' });
      if (!result || result.ok === false) throw new Error(result?.error || 'Hapus gagal (Mungkin item sedang digunakan)');
